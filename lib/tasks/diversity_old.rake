namespace :diversity do
  ...
  task generate: :environment do
    ...
    # Get all the samples for the year and region
    samples_domain = samples_conn.execute("SELECT samples.animal_id, samples.num
     FROM samples WHERE year = #{year} AND region = #{region}")

    # Calculate domain richness
    domain_richness = calculate_richness(samples_domain)

    # Make domain diversity record
    ...
    while strata.length > 0
      ...
      while samples.length > 0
        ...
        strat_id: stratum)
        # Save d if valid, otherwise raise error
        saveIfValid(d)
